...
# -*- coding:utf-8 -*-
# Author:HaujetZhao
# Modified by: junj2ejj
# original code url: https://github.com/HaujetZhao/ytb-vtt-to-srt/blob/master/%E6%B2%B9%E7%AE%A1vtt%E5%8F%8C%E8%A1%8C%E5%AD%97%E5%B9%95%E8%BD%AC%E5%8D%95%E8%A1%8Csrt.py
# modified code url: https://github.com/junj2ejj/CS-lol/ytb-vtt-to-srt.py
# Inherited from ytb-vtt-to-srt project, this code use MPL v2.0 License:

# ==================================
# modification: from original code
# 1: original code are to input target vtt file directory, we revise it to read directory from preset directory.
# 2: we add len(本句字幕内容分行) < 2 to fliter for some unexpected situations
# ==================================
...
# 输出文件 = input('Input the vtt subtitle file path：')
path = os.getcwd()
path = path + "\\lolcs\\description\\"
...
for 输入文件 in pathList:
    输出文件 = path + os.path.splitext(输入文件)[0] + '.srt'
    输入文件 = path + 输入文件
    print('\n\n\n')
